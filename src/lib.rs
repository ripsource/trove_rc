    ...
    impl Barter {
        pub fn new_trade_proposal(
            ...
            // but 50 is a 'nicer' number.
                ...
                assert!(b_nft_len + b_tokens_len + a_tokens_len + a_nft_len <= 50,
...
// user_a_vaults: HashMap<ResourceAddress, Vault> =
            ...
            Runtime::emit_event(PartnerLocked { partner: account_address, partner_badge: partner_nft_badge_resource.clone(), partner_local_id: partner_nft_badge_local.clone()}
            );
            ...
            assert!(!self.swapped, "Swap has occurred already occured");
             ...
             if self.private {
                ...
                if b_badge.is_some() {
                    ...
                    let badge_proof_local = badge_local_to_validate.as_non_fungible().non_fungible_local_id();
        ...
        pub fn partner_deposit_tokens(&mut self, b_token_assets: Vec<Bucket>, b_badge: Option<Proof>) {
            assert!(!self.swapped, "Swap has occurred already occured");
            // if swap required badge
            if self.private {
        ...
        pub fn burn_partner_badge(&mut self, burn_token: Bucket) {
            ...
            assert!(
                &burn_token.as_non_fungible().non_fungible_local_id() == self.badge_partner_local.as_ref().unwrap(),
                "invalid key"
            );
